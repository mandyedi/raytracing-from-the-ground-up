...
GlossySpecular::GlossySpecular(const GlossySpecular& gs) : BRDF(gs), ks(gs.ks), cs(gs.cs), exp(gs.exp) { sampler_ptr = gs.sampler_ptr->clone(); }

GlossySpecular::GlossySpecular(GlossySpecular&& gs) noexcept
    : BRDF(std::move(gs)), ks(std::exchange(gs.ks, 0.0f)), cs(std::move(gs.cs)), exp(std::exchange(gs.exp, 0.0f)), sampler_ptr(std::exchange(gs.sampler_ptr, nullptr)) {}
...
GlossySpecular& GlossySpecular::operator=(GlossySpecular&& gs) noexcept {
...
RGBColor GlossySpecular::sample_f(const ShadeRec& sr, const Vector3D& wo, Vector3D& wi, float& pdf) const {
    ...
    if (sr.normal * wi < 0.0f) {  // reflected ray is below tangent plane
