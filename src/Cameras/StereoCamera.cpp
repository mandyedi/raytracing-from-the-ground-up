...
StereoCamera::StereoCamera(const StereoCamera& c) : Camera(c), viewing_type(c.viewing_type), pixel_gap(c.pixel_gap), beta(c.beta) {
...
StereoCamera& StereoCamera::operator=(const StereoCamera& sc) {
    Camera::operator=(sc);
...
void StereoCamera::render_scene(const World& w, float x /*= 0*/, int offset /*= 0*/) {
    int hres = w.vp.hres;
    int vres = w.vp.vres;
