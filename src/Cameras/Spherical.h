...
public:
    ...
    void render_scene(const World& w, float x = 0, int offset = 0) override;
...
inline void Spherical::set_horizontal_fov(const float fov) { lambda_max = fov / 2.0f; }

inline void Spherical::set_vertical_fov(const float fov) { psi_max = fov / 2.0f; }
