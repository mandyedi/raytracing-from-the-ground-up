...
GeometricObject::GeometricObject(const GeometricObject& go) : color(go.color), shadows(go.shadows) {
...
GeometricObject::GeometricObject(GeometricObject&& go) noexcept
    : color(std::move(go.color)), shadows(std::exchange(go.shadows, true)), material_ptr(std::exchange(go.material_ptr, nullptr)), sampler_ptr(std::exchange(go.sampler_ptr, nullptr)) {}
...
void GeometricObject::set_sampler(Sampler* sPtr) {
    sampler_ptr = sPtr;
    sampler_ptr->map_samples_to_hemisphere(1);  // for perfect diffuse
}

BBox GeometricObject::get_bounding_box() const { return BBox(); }
...
float GeometricObject::pdf(const ShadeRec& sr) { return 0.0f; }
