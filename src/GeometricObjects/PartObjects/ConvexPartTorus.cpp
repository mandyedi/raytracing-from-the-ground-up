...
ConvexPartTorus::ConvexPartTorus(const float _a, const float _b, const float _phi1, const float _phi2, const float _theta1, const float _theta2)
    : GeometricObject(), a(_a), b(_b), phi1(_phi1 * PI / 180.0f), phi2(_phi2 * PI / 180.0f), theta1(_theta1 * PI / 180.0f), theta2(_theta2 * PI / 180.0f), bbox(-a - b, a + b, -b, b, -a - b, a + b) {}

ConvexPartTorus::ConvexPartTorus(const ConvexPartTorus& c) : GeometricObject(c), a(c.a), b(c.b), phi1(c.phi1), phi2(c.phi2), theta1(c.theta1), theta2(c.theta2), bbox(c.bbox) {}
...
ConvexPartTorus& ConvexPartTorus::operator=(const ConvexPartTorus& c) {
...
ConvexPartTorus& ConvexPartTorus::operator=(ConvexPartTorus&& c) noexcept {
...
bool ConvexPartTorus::hit(const Ray& ray, float& tmin, ShadeRec& sr) const {
    ...
    for (int j = 0; j < num_real_roots; j++) {
        if (roots[j] > std::numeric_limits<float>::epsilon()) {
            ...
            if ((roots[j] < t) && ((theta > theta1) && (theta < theta2)) && (((phi > phi1) && (phi < phi2)) || ((phi + TWO_PI) < phi2))) {
                t = roots[j];
                intersected = true;
            }
        }
